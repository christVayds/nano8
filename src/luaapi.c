...
int l_print(lua_State *L){
  ...
  int posx = luaL_optnumber(L, 2, position.x); // default is 0
...
int l_cls(lua_State *L){
  int colorIndex = luaL_optnumber(L, 1, 0); // default clear color is black
...
int l_pget(lua_State *L){
  int posx = luaL_optnumber(L, 1, 0);   // x position 
  int posy = luaL_optnumber(L, 2, 0);   // y position 
...
int l_line(lua_State *L){
  int posx1 = luaL_optnumber(L, 1, 0); 
  int posy1 = luaL_optnumber(L, 2, 0);
...
int l_spr(lua_State *L){
  int sprIndex = luaL_optnumber(L, 1, 0);  // sprite index
  int posx = luaL_optnumber(L, 2, 0);      // position x 
  int posy = luaL_optnumber(L, 3, 0);      // position y
  int width = luaL_optnumber(L, 4, 1);     // width value 
  int height = luaL_optnumber(L, 5, width);    // height value
